<?php
	add_action( 'plugins_loaded', 'init_WC_Gateway_iupay' );

	function init_WC_Gateway_iupay(){
	...
	*/
	class WC_Gateway_iupay extends WC_Payment_Gateway {
		...
		*/
		public function __construct() {
			...
			$this->wooiupayurlko	= $this->get_option( 'wooiupayurlko' );
		...
		* @since 1.0.0
	 	*/
	 	public function admin_options() {
			?>
			<h3><?php _e( 'Iupay', 'caixabank-tools-official' ); ?></h3>
			<p><?php _e( 'Iupay works by sending the user to your bank TPV to enter their payment information.', 'caixabank-tools-official' ); ?></p>
	 	...
	 	function init_form_fields() {
    		$this->form_fields = array(
				'enabled'		 => array(
									...
									'type'			=> 'checkbox',
									'label'			=> __( 'Enable Iupay', 'caixabank-tools-official' ),
									'default'		=> 'no'
									),
				'title'			=> array(
									...
									'default'		=> __( 'Iupay', 'caixabank-tools-official' ),
									'desc_tip'		=> true,
									),
				'description'	=> array(
									...
									'default'		=> __( 'Pay via Iupay; you can pay with your credit card.', 'caixabank-tools-official' )
									),
				'customer'		=> array(
				...
				'debug'			=> array(
									...
									'default'		=> 'no',
									'description'	=> __( 'Log Iupay events, such as notifications requests, inside <code>woocommerce/logs/iupay.txt</code>', 'caixabank-tools-official' ),
									)
									);
    	}
		/**
		...
		*/
		function get_iupay_args( $order ) {
			global $woocommerce;
					...
					if ( $this->wooiupayurlko ){
						if ( $this->wooiupayurlko == 'returncancel' ) {
							$$returnfromiupay = $order->get_cancel_order_url();
						} else {
							$$returnfromiupay = $woocommerce->cart->get_checkout_url();
						}
					} else {
						$$returnfromiupay = $order->get_cancel_order_url();
					}
    	...
    	*/
		function generate_iupay_form( $order_id ) {
			...
			if( !$usesecretsha256 ){
					...
					wc_enqueue_js( '
						jQuery("body").block({
							message: "<img src=\"' . esc_url( apply_filters( 'woocommerce_ajax_loader_url', $woocommerce->plugin_url() . '/assets/images/ajax-loader.gif' ) ) . '\" alt=\"Redirecting&hellip;\" style=\"float:left; margin-right: 10px;\" />'.__( 'Thank you for your order. We are now redirecting you to Iupay to make the payment.', 'caixabank-tools-official' ).'",
							overlayCSS:
							{
								background: "#fff",
								opacity: 0.6
					...
					return '<form action="'.esc_url( $iupay_adr ).'" method="post" id="iupay_payment_form" target="_top">
						' . $form_inputs . '
						<input type="submit" class="button-alt" id="submit_iupay_payment_form" value="'.__( 'Pay with Iupay account', 'caixabank-tools-official' ).'" /> <a class="button cancel" href="'.esc_url( $order->get_cancel_order_url() ).'">'.__( 'Cancel order &amp; restore cart', 'caixabank-tools-official' ).'</a>
					</form>';
			} else {
					$order = new WC_Order( $order_id );
					if ( $this->testmode == 'yes' ):
		...
		*/
		function receipt_page( $order ) {
			echo '<p>'.__( 'Thank you for your order, please click the button below to pay with Iupay.', 'caixabank-tools-official' ).'</p>';
		...
		function successful_request( $posted ) {
			global $woocommerce;
				...
				if ( $response  <= 99 ) {
					...
					//Order cancelled
					$order->update_status( 'cancelled', __( 'Cancelled by Iupay', 'caixabank-tools-official' ) );
					$order->add_order_note( __('Pedido cancelado por Iupay: Tarjeta caducada', 'caixabank-tools-official') );
					WC()->cart->empty_cart();
				} elseif ( $response  == 102 ) {
	...
	function admin_notice_iupay_sha256() {
		...
		if ( $sha->checkfor254testiupay() ) {
			$class = "error";
			$message = __( 'WARNING: You need to add Encryption secret passphrase SHA-256 to Iupay Gateway Settings.', 'caixabank-tools-official' );
	...
	function add_iupay_meta_box(){
		...
		echo '<p><strong>' . __( 'Iupay Date', 'caixabank-tools-official' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_payment_date_iupay', true) . '</p>';
		echo '<p><strong>' . __( 'Iupay Hour', 'caixabank-tools-official' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_payment_hour_iupay', true) . '</p>';
		echo '<p><strong>' . __( 'Iupay Authorisation Code', 'caixabank-tools-official' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_authorisation_code_iupay', true) . '</p>';
		//echo '<p><strong>' . __( 'Redsys Country Card', 'caixabank-tools-official' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_card_country_iupay', true) . '</p>';
		//echo '<p><strong>' . __( 'Redsys Card Type', 'caixabank-tools-official' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_card_type_iupay', true) . '</p>';
	}
